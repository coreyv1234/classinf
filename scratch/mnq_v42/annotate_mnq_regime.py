...
def main():
    ...
    run_hi = grp['high'].cummax().groupby(b['cme_session']).shift(1)
    run_lo = grp['low'].cummin().groupby(b['cme_session']).shift(1)
    sess_open = grp['open'].transform('first')
    b['r_session_loc'] = ((prev_close - run_lo) / (run_hi - run_lo).replace(0, np.nan)).clip(-1, 2)
    ...
    b = b.merge(daily[joincols], on='cme_session', how='left', sort=False)

    b['r_dist_prev_high_atr'] = (b['close'].shift(1) - b['r_prev_d_high']) / b['r_atr14'].replace(0, np.nan)
    b['r_dist_prev_low_atr'] = (b['close'].shift(1) - b['r_prev_d_low']) / b['r_atr14'].replace(0, np.nan)
    b['r_dist_prev_close_atr'] = (b['close'].shift(1) - b['r_prev_d_close']) / b['r_atr14'].replace(0, np.nan)
    ...
    ann = pd.merge_asof(
        t, right,
        left_on='signal_utc', right_on='dt_utc',
    ...
    print({
        'year': args.year,
        'trades': len(ann),
        'regime_cols': len(regime_cols),
        'all_regime_missing_rate': miss,
        'first_signal': str(ann['signal_utc'].min()),
        'last_signal': str(ann['signal_utc'].max())
    })
